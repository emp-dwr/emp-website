...
df_raw_cwq  <- targets::tar_read(df_raw_cwq, store = store)
...
obj_cwq <- BaseClass$new(df_raw_cwq, df_analytes, df_stations)
...
fig_cwq <- WQFigureClass$new(obj_cwq_cur$df_raw, report_year)

# Create RRI DO Object ----------------------------------------------------

obj_rri <- WQRRIClass$new(obj_cwq_cur$df_raw, report_year)
...
create_figs_cwq <- function() {
  ...
  cwq_analytes <- df_analytes %>%
    filter(str_detect(Section, '\\bCEMP\\b')) %>%
    pull(Analyte)
